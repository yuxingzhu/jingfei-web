<template>
  <div class="app-container">
      ...
      <div>
        <el-form ref="searchForm" :inline="true" :model="searchForm" class="wrap">
          <el-form-item label="工艺编号：">
            <el-input v-model="searchForm.craftNo" clearable size="mini" placeholder="请输入工艺编号" style="width: 150px;margin: 0;" @keyup.enter.native="toQuery" />
          </el-form-item>
          <el-form-item label="工艺名称：">
            <el-input v-model="searchForm.craftName" clearable size="mini" placeholder="请输入工艺名称" style="width: 150px;margin: 0;" @keyup.enter.native="toQuery" />
          </el-form-item>
          ...
          <el-form-item label="报工方式：">
            <el-input v-model="searchForm.reporting" clearable size="mini" placeholder="请输入报工方式" style="width: 150px;margin: 0;" @keyup.enter.native="toQuery" />
          </el-form-item>
    ...
    >
      ...
      <el-table-column label="工艺编号" width="190">
        <template slot-scope="scope">
          <span>{{ scope.row.craftNo ? scope.row.craftNo : "--" }}</span>
        </template>
      </el-table-column>
      <el-table-column label="工艺名称" width="190">
        <template slot-scope="scope">
          <span>{{
            scope.row.craftName ? scope.row.craftName : "--"
          }}</span>
        </template>
      </el-table-column>
      <el-table-column label="序号" width="90">
        <template slot-scope="scope">
          <span>{{ scope.row.processId ? scope.row.processId : "--" }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="报工方式" width="190">
        <template slot-scope="scope">
          <span>{{ scope.row.reporting ? scope.row.reporting : "--" }}</span>
        </template>
      </el-table-column>
...
import {
  routingUpload,
  routingQuery
} from '@/api/productionConfiguration/processRoute'
export default {
  name: 'ProcessRoute',
  components: {
  },
  ...
  methods: {
    ...
    routingQuery() {
      this.loading = true
      routingQuery({ ...this.searchForm, page: this.pageOptions.page, size: this.pageOptions.size }).then(res => {
        console.log(res, 'res34343')
        if (res && (res.content !== '' || res.content !== null || res.content !== undefined)) {
          this.routingList = res.content
