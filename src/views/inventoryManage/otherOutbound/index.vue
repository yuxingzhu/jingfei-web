  ...
  <div class="app-container">
      ...
      <div>
        <el-form ref="searchForm" :inline="true" :model="searchForm" class="wrap">
          <el-form-item label="出库编号：">
            <el-input v-model="searchForm.outNo" clearable size="mini" placeholder="请输入出库编号" style="width: 150px;margin: 0;" @keyup.enter.native="toQuery" />
          </el-form-item>
          <el-form-item label="计划入库时间选择：">
            <el-date-picker
              v-model="time"
              size="mini"
              type="datetimerange"
              placeholder="请选择计划入库时间选择"
          ...
          <el-form-item label="出库类型：">
            <el-input v-model="searchForm.outType" clearable size="mini" placeholder="请输入出库类型" style="width: 150px;margin: 0;" @keyup.enter.native="toQuery" />
          </el-form-item>
    ...
    >
      ...
      <el-table-column label="出库日期" width="190">
        <template slot-scope="scope">
          <span>{{ scope.row.outTime ? scope.row.outTime : "--" }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="出库类型" width="190">
        <template slot-scope="scope">
          <span>{{ scope.row.outType ? scope.row.outType : "--" }}</span>
        </template>
      </el-table-column>
...
import {
  otherOutboundUpload,
  otherOutboundQuery
} from '@/api/inventoryManage/otherOutbound'
export default {
  name: 'OtherOutbound',
  components: {
  },
  ...
  methods: {
    ...
    uploadFile(params) {
      ...
      otherOutboundUpload(file).then(res => {
    ...
    otherOutboundQuery() {
      this.loading = true
      otherOutboundQuery({ ...this.searchForm, page: this.pageOptions.page, size: this.pageOptions.size }).then(res => {
