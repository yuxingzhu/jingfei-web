<template>
  <div class="app-container">
      ...
      <div>
        <el-form ref="searchForm" :inline="true" :model="searchForm" class="wrap">
          ...
          <el-form-item label="入库类型：">
            <el-input v-model="searchForm.inType" clearable size="mini" placeholder="请输入入库类型" style="width: 150px;margin: 0;" @keyup.enter.native="toQuery" />
          </el-form-item>
    ...
    >
      ...
      <el-table-column label="入库日期" width="190">
      ...
      <el-table-column label="入库编号" width="190">
        <template slot-scope="scope">
          <span>{{
      ...
      <el-table-column label="入库类型" width="190">
        <template slot-scope="scope">
          <span>{{ scope.row.inType ? scope.row.inType : "--" }}</span>
        </template>
      </el-table-column>
...
import {
  otherWarehousingUpload,
  otherWarehousingQuery
} from '@/api/inventoryManage/otherIncomingGoods'
export default {
  ...
  methods: {
    ...
    uploadFile(params) {
      ...
      otherWarehousingUpload(file).then(res => {
    ...
    otherWarehousingQuery() {
      this.loading = true
      otherWarehousingQuery({ ...this.searchForm, page: this.pageOptions.page, size: this.pageOptions.size }).then(res => {
