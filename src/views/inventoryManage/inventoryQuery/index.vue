<template>
  <div class="app-container">
      ...
      <div>
        <el-form ref="searchForm" :inline="true" :model="searchForm" class="wrap">
          <el-form-item label="物料编号：">
            <el-input v-model="searchForm.materialNo" clearable size="mini" placeholder="请输入物料编号" style="width: 150px;margin: 0;" @keyup.enter.native="toQuery" />
          </el-form-item>
    ...
    >
      ...
      <el-table-column label="物料分类" width="100">
        <template slot-scope="scope">
          <span>{{
      ...
      <el-table-column label="规格/尺寸" width="150">
        <template slot-scope="scope">
          <span>{{
      ...
      <el-table-column label="库存数量" width="100">
        <template slot-scope="scope">
          <span>{{
            scope.row.inventoryNumber ? scope.row.inventoryNumber : "--"
          }}</span>
        </template>
      </el-table-column>
      <el-table-column label="安全库存上限" width="150">
        <template slot-scope="scope">
          <span>{{
            scope.row.safetyStockUp ? scope.row.safetyStockUp : "--"
          }}</span>
        </template>
      </el-table-column>
      <el-table-column label="安全库存下限" width="150">
        <template slot-scope="scope">
          <span>{{
            scope.row.safetyStockDown ? scope.row.safetyStockDown : "--"
          }}</span>
        </template>
      </el-table-column>
...
import {
  inventoryUpload,
  inventoryQuery
} from '@/api/inventoryManage/inventoryQuery'
export default {
  name: 'InventoryQuery',
  components: {
  },
  ...
  methods: {
    ...
    inventoryQuery() {
      this.loading = true
      inventoryQuery({ ...this.searchForm, page: this.pageOptions.page, size: this.pageOptions.size }).then(res => {
