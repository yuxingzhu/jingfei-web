      ...
      <div>
        <el-form ref="searchForm" :inline="true" :model="searchForm" class="wrap">
          <el-form-item label="工序编号">
            <el-input v-model="searchForm.processNo" clearable size="mini" placeholder="请输入工序编号" style="width: 150px;margin: 0;" @keyup.enter.native="toQuery" />
          </el-form-item>
          <el-form-item label="工序名称">
            <el-input v-model="searchForm.processName" clearable size="mini" placeholder="请输入工序名称" style="width: 150px;margin: 0;" @keyup.enter.native="toQuery" />
          </el-form-item>
    ...
    >
      ...
      <el-table-column label="工序编号">
        <template slot-scope="scope">
          <span>{{ scope.row.processNo ? scope.row.processNo : "--" }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="不良品项">
        <template slot-scope="scope">
          <span>{{ scope.row.defectiveItems ? scope.row.defectiveItems : "--" }}</span>
        </template>
      </el-table-column>
...
import {
  processUpload,
  processQuery
} from '@/api/productionConfiguration/processDefinition'
export default {
  ...
  methods: {
    ...
    processQuery() {
      this.loading = true
      processQuery({ ...this.searchForm, page: this.pageOptions.page, size: this.pageOptions.size }).then(res => {
        console.log(res, 'res34343')
        if (res && (res.content !== '' || res.content !== null || res.content !== undefined)) {
          this.processDefinitionList = res.content
