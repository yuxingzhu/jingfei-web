...
export default {
  ...
  methods: {
    ...
    // 获取开工信息
    construction() {
      construction({ productionName: 'A' }).then(res => {
    ...
    // 获取实时报工
    reportingByProduction() {
      reportingByProduction({ productionName: 'A' }).then(res => {
        if (res && Number(res.code) === 200) {
    ...
    // 获取综合信息
    information() {
      information({ productionName: 'A' }).then(res => {
        if (res && Number(res.code) === 200) {
    ...
    // 获取产品完成情况
    productCompletion() {
      productCompletion({ productionName: 'A' }).then(res => {
        if (res && Number(res.code) === 200) {
    ...
    // 获取物料信息
    materialInformation() {
      materialInformation({ productionName: 'A' }).then(res => {
        if (res && Number(res.code) === 200) {
