<template>
  <div class="app-container">
      ...
      <div>
        <el-form ref="searchForm" :inline="true" :model="searchForm" class="wrap">
          <el-form-item label="供应商编号：">
            <el-input v-model="searchForm.supplierNo" clearable size="mini" placeholder="请输入供应商编号" style="width: 150px;margin: 0;" @keyup.enter.native="toQuery" />
          </el-form-item>
          <el-form-item label="供应商名称：">
            <el-input v-model="searchForm.supplierName" clearable size="mini" placeholder="请输入供应商名称" style="width: 180px;margin: 0;" @keyup.enter.native="toQuery" />
          </el-form-item>
          <el-form-item label="联系人：">
            <el-input v-model="searchForm.contacts" clearable size="mini" placeholder="请输入联系人" style="width: 150px;margin: 0;" @keyup.enter.native="toQuery" />
          </el-form-item>
    ...
    >
      ...
      <el-table-column label="供应商编号" width="190">
        <template slot-scope="scope">
          <span>{{
            scope.row.supplierNo ? scope.row.supplierNo : "--"
          }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="联系电话">
        <template slot-scope="scope">
          <span>{{ scope.row.contactsPhone ? scope.row.contactsPhone : "--" }}</span>
        </template>
      </el-table-column>
      <el-table-column label="采购员">
        <template slot-scope="scope">
          <span>{{
            scope.row.purchaser ? scope.row.purchaser : "--"
          }}</span>
        </template>
      </el-table-column>
...
import {
  supplierManagementUpload,
  supplierManagementQuery
} from '@/api/merchantManage/supplierManage'
export default {
  name: 'SupplierManage',
  components: {
  },
  ...
  methods: {
    ...
    uploadFile(params) {
      ...
      supplierManagementUpload(file).then(res => {
    ...
    supplierManagementQuery() {
      this.loading = true
      supplierManagementQuery({ ...this.searchForm, page: this.pageOptions.page, size: this.pageOptions.size }).then(res => {
