<template>
  <div class="app-container">
      ...
      <div>
        <el-form ref="searchForm" :inline="true" :model="searchForm" class="wrap">
          ...
          <el-form-item label="结算状态：">
            <el-select v-model="searchForm.settlementStatus" placeholder="请选择结算状态" size="mini">
              <el-option
          ...
          <el-form-item label="类别：">
            <el-select v-model="searchForm.type" placeholder="请选择类别" size="mini">
              <el-option
    ...
    >
      ...
      <el-table-column label="退货单号" width="190">
        <template slot-scope="scope">
          <span>{{
            scope.row.returnOrderNumber ? scope.row.returnOrderNumber : "--"
          }}</span>
        </template>
      </el-table-column>
      <el-table-column label="关联采购入库单" width="190">
        <template slot-scope="scope">
          <span>{{ scope.row.relatedProcurement ? scope.row.relatedProcurement : "--" }}</span>
        </template>
      </el-table-column>
      <el-table-column label="结算状态" width="190">
        <template slot-scope="scope">
...
import {
  purchaseReturnUpload,
  purchaseReturnQuery
} from '@/api/purchasingManage/purchaseReturn'
export default {
  name: 'PurchaseReturn',
  components: {
  },
  ...
  methods: {
    ...
    uploadFile(params) {
      ...
      purchaseReturnUpload(file).then(res => {
    ...
    purchaseReturnQuery() {
      this.loading = true
      purchaseReturnQuery({ ...this.searchForm, page: this.pageOptions.page, size: this.pageOptions.size }).then(res => {
