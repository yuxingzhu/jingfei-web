      ...
      <div>
        <el-form ref="searchForm" :inline="true" :model="searchForm" class="wrap">
          <el-form-item label="工具编号：">
            <el-input v-model="searchForm.deviceNo" clearable size="mini" placeholder="请输入工具编号" style="width: 150px;margin: 0;" @keyup.enter.native="toQuery" />
          </el-form-item>
...
import {
  deviceUpload,
  deviceQuery
} from '@/api/equipmentManage/toolsEquipmentList'
export default {
  ...
  methods: {
    ...
    deviceQuery() {
      this.loading = true
      deviceQuery({ ...this.searchForm, page: this.pageOptions.page, size: this.pageOptions.size }).then(res => {
        console.log(res, 'res34343')
        if (res && (res.content !== '' || res.content !== null || res.content !== undefined)) {
          this.toolsEquipmentList = res.content
