<template>
  <div class="app-container">
      ...
      <div>
        <el-form ref="searchForm" :inline="true" :model="searchForm" class="wrap">
          <el-form-item label="采购单号：">
            <el-input v-model="searchForm.purchaseOrderNo" clearable size="mini" placeholder="请输入采购单号" style="width: 150px;margin: 0;" @keyup.enter.native="toQuery" />
          </el-form-item>
          <el-form-item label="供应商名称：">
            <el-input v-model="searchForm.supplierName" clearable size="mini" placeholder="请输入供应商名称" style="width: 150px;margin: 0;" @keyup.enter.native="toQuery" />
          </el-form-item>
          <el-form-item label="采购员：">
            <el-input v-model="searchForm.purchaser" clearable size="mini" placeholder="请输入采购员" style="width: 150px;margin: 0;" @keyup.enter.native="toQuery" />
          </el-form-item>
          <el-form-item label="采购时间选择：">
            <el-date-picker
              v-model="time"
              size="mini"
              type="datetimerange"
    ...
    >
      ...
      <el-table-column label="类别" width="190">
        <template slot-scope="scope">
          <span>{{
      ...
      <el-table-column label="供应商名称" width="190">
        <template slot-scope="scope">
          <span>{{
      ...
      <el-table-column label="采购日期" width="190">
        <template slot-scope="scope">
          <span>{{ scope.row.purchaseTime ? scope.row.purchaseTime : "--" }}</span>
        </template>
      </el-table-column>
      <el-table-column label="采购员 " width="190">
        <template slot-scope="scope">
          <span>{{ scope.row.purchaser ? scope.row.purchaser : "--" }}</span>
        </template>
      </el-table-column>
      <el-table-column label="采购数量" width="190">
        <template slot-scope="scope">
          <span>{{
      ...
      <el-table-column label="入库数量" width="190">
        <template slot-scope="scope">
      ...
      <el-table-column label="物料编号" width="190">
        <template slot-scope="scope">
          <span>{{ scope.row.materialNo ? scope.row.materialNo : "--" }}</span>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    uploadFile(params) {
      ...
      purchaseOrderUpload(file).then(res => {
    ...
    purchaseOrderQuery() {
      this.loading = true
      purchaseOrderQuery({ ...this.searchForm, page: this.pageOptions.page, size: this.pageOptions.size }).then(res => {
